import { f as useRuntimeConfig, h as getHeader, e as createError } from './nitro.mjs';
...
async function requireUserRole(event, allowedRoles) {
  const client = await serverSupabaseServiceRole(event);
  const authHeader = getHeader(event, "authorization");
  if (!authHeader) {
    throw createError({ statusCode: 401, statusMessage: "N\xE3o autenticado" });
  }
  const token = authHeader.replace("Bearer ", "");
  const { data: userData, error: userError } = await client.auth.getUser(token);
  if (userError || !userData.user) {
    throw createError({ statusCode: 401, statusMessage: "Usu\xE1rio logado inv\xE1lido ou token expirado" });
  }
  const userId = userData.user.id;
  const { data: profile, error: profileError } = await client.from("profiles").select("role").eq("id", userId).single();
  if (profileError || !profile) {
    throw createError({ statusCode: 403, statusMessage: "Sem permiss\xE3o (perfil n\xE3o encontrado)" });
  }
  const role = profile.role;
  return { user: userData.user, role, client };
}

export { requireUserRole as r };
